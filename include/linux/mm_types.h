...
struct vm_area_struct {
	 ...
	 */
	pgprot_t vm_page_prot; // 页表中关于内存页的访问权限就是由 vm_page_prot 决定的
...
struct mm_struct {
	struct {
...
#endif
		unsigned long mmap_base;	/* base of mmap area */  // 内存映射区的起始地址
		...
		unsigned long total_vm;	   /* Total pages mapped */ // 表示在进程虚拟内存空间中总共与物理内存映射的页的总数
		unsigned long locked_vm;   /* Pages that have PG_mlocked set */ // 表示既不能换出，也不能移动的内存页总数
		atomic64_t    pinned_vm;   /* Refcount permanently increased */ // 表示既不能换出，也不能移动的内存页总数
